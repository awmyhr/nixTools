...
#   retrieve all the files on all the systems over a secure 
#   connection (ssh/scp/sftp).  This will use rsync to do the work.
# The structure/code is based on SyncHome.pl, which is in turn
#   based on allservers.pl.
#
...
use Shell qw( scp );			#easy use of scp
...
my(
	...
	$WorkDir,			#Var: directory to hold captured files
...
$FileList	= "$ENV{HOME}/etc/getfile.list";
$WorkDir	= "$ENV{HOME}/collect";
$HostReport	= "This file get done at: " . localtime(time) . ".\n\n";
...
$Version  
By: awmyhr
Date Created: December 30, 2002  Last Modified: $Modified
END_of_text
	exit 0;
}
...
while (<FILELIST>) {
	...
	chomp($_);
	$HostReport .= $_ . " will be retrieved.\n";
...
while (<HOSTLIST>){			#This is where all the work is done
	...
	if( (!$opt_e || lc($HostInfo[1])=~/$opt_e/) && (!$opt_p || lc($HostInfo[2])=~/$opt_p/)){
		...
		foreach $File (@Files) {
			if($Dev) { 
				print "scp $UserID\@$HostInfo[0]:$File $WorkDir$File.$HostInfo[0] \n"; 
			}
			
			scp( "$UserID\@$HostInfo[0]:$File", "$WorkDir$File.$HostInfo[0]" );
		};	
